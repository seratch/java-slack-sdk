...
public interface SocketModeClient extends Closeable {
    ...
    /**
     * The message processor implementation. The default one ("Default") is stable,
     * but it can be slow to consume received Socket Mode messages
     * especially when your listeners take time before returning ack().
     * If you need much faster one, try "Fast" instead.
     */
    enum MessageProcessorMode {
        /**
         * The simple implementation, which consumes incoming Socket Mode messages at a constant pace.
         */
        Default,
        /**
         * The latest implementation, which consumes messages from the internal queue concurrently.
         */
        Fast,
    }

    static int getConcurrency(MessageProcessor messageProcessor) {
        return (messageProcessor != null && messageProcessor.getConcurrency() != null) ?
                messageProcessor.getConcurrency() :
